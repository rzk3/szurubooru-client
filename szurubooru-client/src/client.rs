...
impl<'a> SzurubooruRequest<'a> {
    ...
    /// Update an existing post based on the `contentUrl` field, which the server will use to
    /// download and update the post
    ...
    async fn create_update_post_from_file(
        &self,
        file: &mut File,
        ...
        if let Some(thumbnail) = thumbnail {
            let thumbnail_part = self.part_from_file(thumbnail)?;
    ...
    /// See [SzurubooruRequest::create_post_from_url] for more details about the fields in
    /// (CreateUpdatePost)[models::CreateUpdatePost]
    pub async fn create_post_from_file(
        &self,
        file: &mut File,
        thumbnail: Option<&mut File>,
        new_post: &CreateUpdatePost,
    ) -> SzurubooruResult<PostResource> {
        self.create_update_post_from_file(file, thumbnail, "/posts", Method::POST, new_post)
            .await
    }
    ...
    pub async fn create_post_from_file_path(
        ...
        self.create_post_from_file(&mut file, thumbnail_file.as_mut(), new_post)
            .await
    }
    ...
    pub async fn update_post_from_file(
        ...
        let path = format!("/posts/{post_id}");
        self.create_update_post_from_file(file, thumbnail, &path, Method::PUT, update_post)
            .await
    }
    ...
    pub async fn update_post_from_file_path(
        ...
        self.update_post_from_file(post_id, &mut file, thumbnail_file.as_mut(), update_post)
            .await
    }
    ...
    pub async fn delete_pool_category<T>(&self, category_name: T) -> SzurubooruResult<()>
    where
        T: AsRef<str> + Display,
    {
        let path = format!("/pool-category/{category_name}");
